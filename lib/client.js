...
.dshsp-bgbar { display: flex; align-items: center; gap: 8px; margin: 0 0 10px; font-size: 12px; }
.dshsp-bgbar-label { color: var(--dsw-alias-label-secondary); }
.dshsp-bgbar-clear {
  border: none;
  background: transparent;
...
.dshsp-bgbar-clear:hover { text-decoration: underline; }
`;
    ...
    function apply(ctx) {
      ...
      function readStored() {
        try {
          ...
          if (typeof v === 'string' && v.indexOf('nl:') === 0) return v;
        } catch (e) {}
        return 'default';
      }
      ...
      function readBg() {
        try { return localStorage.getItem(BG_KEY) || null; } catch (e) { return null; }
      }
      function writeBg(src) {
        try {
          if (src) localStorage.setItem(BG_KEY, src);
          else localStorage.removeItem(BG_KEY);
        } catch (e) {}
      }
      var currentBg = readBg();
      ...
      function applyDerivedCss() {
        ...
        for (var i = 0; i < els.length; i++) {
          ...
          if (currentBg) {
            var veil = scheme === 'dark' ? 'rgba(9,11,15,0.6)' : 'rgba(255,255,255,0.55)';
            el.style.setProperty('--dsw-alias-bg-base', 'linear-gradient(' + veil + ',' + veil + '), url("' + currentBg + '") center / cover no-repeat fixed', 'important');
          } else if (layer1 !== undefined && base !== undefined) {
            if (scheme === 'dark') {
      ...
      function SkinRow() {
        ...
        var bgState = React.useState(readBg());
        var bg = bgState[0];
        var setBg = bgState[1];
          ...
          open
            ? el('div', { className: 'dshsp-body' },
                ...
                el('div', { className: 'dshsp-img' },
                  ...
                  el('button', { className: 'dshsp-nl-btn', type: 'button', onClick: loadUrl }, '设为背景'),
                ),
                bg
                  ? el('div', { className: 'dshsp-bgbar' },
                      el('span', { className: 'dshsp-bgbar-label' }, '已设置背景图片'),
                      el('button', { className: 'dshsp-bgbar-clear', type: 'button', onClick: clearBackground }, '清除'),
                    )
                  : null,
